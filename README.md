...
一个使用 Kotlin、Jetpack Compose 和 Material Design 3 构建的原生安卓本地漫画阅读器。简体中文界面，无账号、无广告，不申请联网权限，漫画和阅读记录都留在设备上。

## 安装与开始阅读

安装包位于 `dist/ZViewer-1.0.1-release.apk`，使用本机独立签名密钥签名，可直接安装。支持 Android 10 及以上，目标平台为 Android 16 / API 36。通用安装包包含 **arm64-v8a**（现代安卓手机）与 **x86_64**（模拟器）。1.0.1 更换双色 Z 矢量图标并精简界面文案，沿用 1.0.0 的签名，可直接覆盖升级并保留书库与阅读记录。
...
Android 通常不允许授权整个内部存储根目录、整个 Download 根目录或 Android/data。请创建一个具体的漫画子文件夹，例如 `Download/漫画`，再选择它。应用无需「所有文件访问」权限。
...
| 动态图片 | APNG（包括带动画的 `.png`）、Animated WebP、GIF、Animated AVIF |
...
| 存储 | 私有解压与 PDF 缓存、缓存清理、约 1.5 GiB 自动回收阈值、不修改原文件 |

封面以最高 1000 像素宽度生成并缓存。阅读图片按窗口尺寸的两倍请求解码，宽度上限 3200 像素，结合像素预算控制内存；PDF 以最高 2560 像素宽度渲染。缩放范围为适合屏幕至 5 倍，并可缩回适合屏幕。

## 格式边界

- RAR 读取由 libarchive 提供，包含 RAR4 / RAR5；7Z 固实压缩包首次打开需要完整准备图片。
- 加密压缩包、分卷文件、密码 PDF 和 DRM 内容不属于此版本的支持范围。请先解密或解压为图片文件夹。
- HEIF / HEIC 使用安卓系统解码器，具体编码、位深及色彩配置支持范围取决于设备；此版本不将 HEIF 多图集合或动态 HEIF 作为动画播放。
- 动态 AVIF 使用随包提供的 libavif；APNG 使用 APNG4Android。GIF 与动态 WebP 使用系统 `ImageDecoder`，不会降级为首帧静图。
- 图片文件夹中的直接图片组成一本书；子目录各自扫描。压缩包中的图片按完整路径自然排序，因此 `2.jpg` 位于 `10.jpg` 前面。
...
此次交付的签名密钥位于 `.signing/zviewer-release.jks`，别名为 `zviewer`。该目录已排除在 Git 外，文件权限为仅当前用户可读写。请单独备份这个密钥；以后使用同一密钥和更高 `versionCode` 发布，才能保留已安装应用的数据直接升级。

当前本地密钥使用构建配置中的本地默认口令。正式维护时可更改口令，并通过 `ZVIEWER_STORE_PASSWORD` 与 `ZVIEWER_KEY_PASSWORD` 传入；不要将私钥提交到仓库。

在新的机器首次构建时，先恢复原密钥；若只是独立试用，也可以生成自己的密钥，但它不能覆盖安装本次签名的 APK。

```sh
mkdir -p .signing
keytool -genkeypair -keystore .signing/zviewer-release.jks \
  -alias zviewer -keyalg RSA -keysize 4096 -validity 10000 \
  -storepass zviewer-local-release -keypass zviewer-local-release \
...
生成测试样本需要 Pillow、pillow-heif、reportlab、py7zr、文泉驿字体与 RAR 命令。此环境的依赖已安装在 `~/.cache/zviewer-test-venv/`，测试用 RAR 命令位于 `/tmp/rar/rar`。
...
项目沿用仓库原有 Apache 2.0 许可证。使用 AndroidX / Compose、Kotlin、libarchive-android、libarchive、APNG4Android 和 libavif。已参考相邻目录 `APNG4Android` 的实现与接入文档，实际构建使用 Maven Central 的固定稳定版本，不依赖相邻仓库路径。
